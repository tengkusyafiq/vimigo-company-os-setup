...
restart_bridge() {
    ...
    local root="${1:-}" port="${2:-}" waited=0 freeing=0
    suspend_bridge "$root" >/dev/null 2>&1

    # Waited for the port to come free before starting the replacement.
    #
    # Stopping the bridge returns before the listening socket is released, and a
    # replacement started into that gap dies on the port already being in use -
    # observed on Windows in a real bridge.log, and the same race exists here.
    # Worse than the crash is what follows: the wait below then sees the OLD
    # process still answering and returns happily, so a restart that failed
    # reports success and the loop polls a bridge that is not there.
    while [ "$freeing" -lt 20 ]; do
        bridge_reachable "$port" || break
        sleep 1
        freeing=$((freeing + 1))
    done
...
pairing_body() {
    # $4 is the number the code was issued against, shown beside it.
    #
    # A mistyped number is the one failure here with no symptom: the bridge
    # accepts it, WhatsApp sends the code to a stranger's phone, and the
    ...
    if [ -n "$code_for" ]; then
        code_for_line="
$(cat <<CODEFOR
      <p class="calm">That code went to $code_for. Not your number? Close this
      window and start again.</p>
CODEFOR
)"
    fi
...
to_e164() {
    ...
    # Every shape a Malaysian owner might write, and dashes, spaces,
    # brackets and a leading + are all thrown away before anything else
    # looks at it:
    #
    #   012-345 6789      0123456789      +60 12 345 6789
    #   60123456789       +60123456789    123456789
    #
    # The question asks for one example and accepts all of them, rather
    # than explaining a format to somebody who has never been asked for one.
    local digits rest
    digits="$(printf '%s' "${1:-}" | tr -cd '0-9')"
    # A mobile with its trunk zero already dropped - what people write when
    # they have been told "no leading zero" and stopped there. Malaysian
    # mobiles are the only ones this could be: 1 followed by 8 or 9 digits.
    # Singapore starts 8 or 9, Indonesia 8, so nothing else is caught by
    # accident.
    case "$digits" in
        1[0-9][0-9][0-9][0-9][0-9][0-9][0-9]|1[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9])
            printf '60%s' "$digits"; return 0 ;;
    esac
    case "$digits" in
        60*)
            rest="${digits#60}"
